...
from controllers.expenses_controller import ingresar_gasto, mostrar_resumen
...
def iniciar_interfaz():
    ...
    # Logo
    logo_image = Image.open("assets/logo.png")  # Asegúrate de tener un logo en la carpeta assets
    ...
    # Botones
    ttk.Button(root, text="Ingresar Gasto", command=lambda: ingresar_gasto(entry_nombre, entry_cantidad, categoria_var)).grid(row=4, column=0, columnspan=2, padx=10, pady=10)
    ...
    ttk.Button(root, text="Salir", command=root.quit).grid(row=6, column=0, columnspan=2, padx=10, pady=10)
